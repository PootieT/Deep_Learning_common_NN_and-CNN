...
def main():
    ...
    variable_summaries(z_conv1, 'z_conv1')
    ...
    variable_summaries(z_conv2, 'z_conv2')
    ...
    variable_summaries(z_fc1, 'z_fc1')
    ...
    train_step = tf.train.MomentumOptimizer(1e-4,0.01).minimize(cross_entropy)
    correct_prediction = tf.equal(tf.argmax(y_conv,1), tf.argmax(y_,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    # setup validation and test summary
    ...
    valid_test_writer = tf.summary.FileWriter(result_dir)
    ...
    for i in range(max_step):
        ...
        validation_set = mnist.validation                                    # the batch size is 50
        test_set = mnist.test
        if i%100 == 0:
        ...
        if i % 1100 == 0 or i == max_step:
            ...
            valid_acc, valid_summ = sess.run([accuracy, validation_summary],
            	feed_dict={x: validation_set.images, y_: validation_set.labels, keep_prob: 1})
            valid_test_writer.add_summary(valid_summ, i)

    		# updating testing summary
            test_acc, test_summ = sess.run([accuracy, test_summary],
            	feed_dict={x: test_set.images, y_: test_set.labels, keep_prob: 1})
            valid_test_writer.add_summary(test_summ, i)
            valid_test_writer.flush()
